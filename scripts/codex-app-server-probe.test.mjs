...
if (process.argv[2] !== "--fake-server") test("attestation records only live probe evidence", async () => {
  ...
  try {
    ...
    assert.equal(attestation.schema, "piku.codex-write-attestation.v5");
    ...
    assert.equal(typeof attestation.child_environment.PATH, "string");
    assert.equal(attestation.child_environment.HOME, undefined);
    assert.equal(attestation.child_environment.CODEX_HOME, undefined);
